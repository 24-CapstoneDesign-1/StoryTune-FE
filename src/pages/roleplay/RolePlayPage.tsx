...
const RolePlayPage: React.FC = () => {
  ...
  const [page, setPage] = useState<number>(0);
    ...
    useEffect(() => {
      ...
      console.log("My Book ID:", location.state.myBookId);
      if (location.state.myBookId) {
        getDetail();
      }
    }, [location.state]);
    ...
    const getDetail = async () => {
      try {
        const data = await bookService.myBookDetail(location.state.myBookId);
  ...
  const [bookContent, setBookContent] = useState<Book.MyMakedRes['result'] | null>(null);
